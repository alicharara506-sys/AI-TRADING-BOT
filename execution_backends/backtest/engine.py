...
from execution_backends.backtest.commission import CommissionModel, ZeroCommission


class BacktestExecutionEngine:
    """Backtest fill model: market orders fill instantly at the current bid/ask
    (spread cost) plus a pluggable commission cost; no latency or partial fills
    yet -- those arrive as the Backtesting Engine gains further fidelity.
    ...
    def __init__(
        self, event_bus: EventBus, *, commission_model: CommissionModel | None = None
    ) -> None:
    ...
    async def submit_order(self, request: OrderRequest) -> OrderAck:
        ...
        self._next_order_id += 1
        commission = self._commission_model.calculate(volume=request.volume, price=fill_price)
