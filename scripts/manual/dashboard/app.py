...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Show a banner marking every value on this page as synthetic demo data.",
    )
...
def main() -> None:
    ...
    if args.demo:
        st.warning(
            "**DEMO MODE** -- every bar, signal, and balance on this page is "
            "synthetic (random-walk) data generated by "
            "scripts/manual/seed_demo_data.py, not a real account or real "
            "market prices. Signals are real output from the real "
            "SignalFusionStrategy, just fed fake input to preview the UI.",
            icon="⚠️",
        )
    st.caption(
